...
industry_codes <- tables[[2]]

clean_industry_codes <- industry_codes %>%
  clean_names() %>%
  slice(-21)
...
links <- url %>%
  read_html() %>%
  html_elements(xpath = '//*[@id="main-content-td"]') %>%
  html_elements("ul li a") %>%
  html_attr("href") %>%
  as_tibble() %>%
  mutate(sector = str_remove_all(value,"iag|.htm")) %>%
  mutate(xpath_id = paste0("iag",sector,"emp1")) %>%
  inner_join(clean_industry_codes) %>%
  arrange(sector) %>%
  mutate(value = url_absolute(value,"https://www.bls.gov/iag/tgs/")) %>%
  #select(value) %>%
  slice(4:5) #%>%
  #as_vector()
...
for (row in links) {
  ...
  #temp <- url %>%
  #  read_html() %>%
  #  html_elements(xpath = paste0('//*[@id="',xpath_id,'"]'))
...
Employment, all employees (seasonally adjusted)
  ...
  filter(str_detect(sector,"-") | str_length(sector) == 2)%>%
  anti_join(clean_industry_codes)
  
  
  
  
  filter(str_detect(value,"iag[0-9][0-9]\\.htm")) %>%
  arrange(value)
  as_vector() %>%
  unname()
