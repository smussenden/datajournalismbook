...
There are totals for all of Maryland's jurisdictions, plus one for "Unknown" and one that's NA. All but the last one have roughly the same number of records (between 315-335). What's going on with the NA records?
...
```{r}
datatable(county_vaccinations_by_date) %>% formatDate(2, "toLocaleDateString")
```

Let's just look at the most recent date, and that's something that takes on different meaning when we're talking about updating data. We need to make this generic so that every time we pull this up and run it, it's the most recent date at the top. This time, it's very simple.
...
One thing we are missing? Automated context. What if we programmatically wrote the chatter for this chart using the percent change calculation we did before?
...
```{r}
sub <- paste("The number of people in Prince George's County fully vaccinated on ", format(latest_date$vaccination_date, format="%B %d"), " ", direction, " by ", changetext, " percent compared to a week ago", sep="")
```
