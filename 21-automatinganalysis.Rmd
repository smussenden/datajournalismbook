...
The global COVID-19 pandemic is something we're going to be writing about and covering for some time. Two elements of it are reports of cases and data on vaccinations. Both are reported frequently (mostly daily), and we'll be talking about them for some time. So they are ideal candidates for repeating analysis -- scripting the questions we want to answer every week and doing so in a way that we can just load it without having to change anything.

Let's get some other libraries to our typical tidyverse import. First, I'm going to add `rvest` so we can scrape some HTML tables. The next one I'm going to add is `DT`. It stands for datatables, and it makes your dataframes into html tables that are browsable and searchable. This is a bigger issue for me -- the author who is turning these into html pages -- than you, working in your notebooks. We're also going to add a library called `ggrepel`, which assists in putting tables on dots in charts.

You install them the same way you do anything else -- `install.packages("rvest")` and `install.packages("DT")` and `install.packages("ggrepel")`.
...
Maryland [publishes data daily on COVID-19 cases and deaths on the state Department of Heath website](https://coronavirus.maryland.gov/).
...
But there's a catch. When you try to use rvest to scrape this HTML table, you won't see the actual numbers. That's because the data is getting populated on the web page _after_ the page loads in a browser. The good news is that the data has to come from some place, and in this case it's a JSON file. JSON is basically a dictionary object: it has keys and values. A sample JSON file might look like this:
...
The keys in this file are `university` and `college`, and the values are `University of Maryland` and `Merrill College of Journalism`. You get to the values by knowing the keys.
...
Next we'll read the JSON file from Maryland and use `clean_names()` to make the columns lowercase:
...
We need to clean this dataframe a bit more, though, to remove those colons from `description` and to make the other columns into numeric data (and remove the commas). We'll use `str_replace` and `str_replace_all` to do that.
...
Finally, let's break this data out into different kinds: counties, age, race and other totals. To do this, we'll filter on description and add today's date:

```{r}
covid_race_ethnicity <- covid_data %>%
  filter(description == 'African-American (NH)' | description == 'Asian (NH)' | description == 'White (NH)' | description == 'Hispanic' | description == 'Other (NH)' | description == 'Race and Ethnicity Data not available') %>%
  mutate(date='2021-11-10')

covid_age <- covid_data %>%
  filter(description == '0-9' | description == '10-19' | description == '20-29' | description == '30-39' | description == '40-49' | description == '50-59' | description == '60-69' | description == '70-79' | description == '80+' | description == 'Age Data Not Available') %>%
  mutate(date='2021-11-10')

covid_gender <- covid_data %>%
  filter(description == 'Female' | description == 'Male' | description == 'Unknown Gender') %>%
  mutate(date='2021-11-10')

covid_hospitalized <- covid_data %>%
  filter(description == 'Currently hospitalized' | description == 'Acute care' | description == 'Intensive care') %>%
  mutate(date='2021-11-10')
